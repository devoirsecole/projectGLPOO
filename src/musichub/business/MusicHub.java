...
public class MusicHub {
	...
	private List<AudioElement> history;
	...
	public static final String HISTORY_FILE_PATH = DIR + "\\files\\elements.xml";
	...
	public MusicHub () {
		...
		history = new LinkedList<AudioElement>();
		this.loadElements();
		this.loadAlbums();
		this.loadPlaylists();
	}
	...
	public void addToHistory(AudioElement element){
	...
	public String seeHistory(){
		...
		for (int i = 0 ; i < this.history.size(); i++){
			...
			history = history + i + " - " +this.history.get(i).getTitle()+ "\n";
		}
	...
	private void loadHistory(){
		...
		NodeList historyNodes = xmlHandler.parseXMLFile(HISTORY_FILE_PATH);
		if (historyNodes == null) return;

		for (int i = 0; i < historyNodes.getLength(); i++) {
			if (historyNodes.item(i).getNodeType() == Node.ELEMENT_NODE)   {
				Element historyElement = (Element) historyNodes.item(i);
				if (historyElement.getNodeName().equals("history")) 	{
					try {
						//this.addAlbum(new Album (historyElement));
					} catch (Exception ex) {
						System.out.println ("Something is wrong with the XML history element");
					}
				}
			}
		}
	...
	public void saveElements() {
 	...
 	private void saveHistory(){
		...
		//save all AudioElements
		Iterator<AudioElement> historysIter = history.listIterator();
		while (historysIter.hasNext()) {

			AudioElement currentElement = historysIter.next();

			currentElement.createXMLElement(document, root);

		}
		xmlHandler.createXMLFile(document, HISTORY_FILE_PATH);

	}

 	public AudioElement playAudioElement(String toplay) throws NoElementFoundException{
		...
		for ( i =0 ; i < elements.size(); i++){
			if(elements.get(i).getTitle().toLowerCase().equals(toplay) ){
				found = true;
				break;
			}
		}
		...
		if (found){
			elements.get(i).playAUdio();
			return elements.get(i);
		}

		else throw new NoElementFoundException("Element " + toplay + " not found!");

	}

	public void showLyrics(String toshow) throws NoElementFoundException, FileNotFoundException {

		toshow = toshow.toLowerCase();

		boolean found = false;
		int i ;

		for ( i =0 ; i < elements.size(); i++){
			if(elements.get(i).getTitle().toLowerCase().equals(toshow) ){
				found = true;
				break;
			}
		}
		...
		if (found){
			((Song)elements.get(i)).seeLyrics();
		}

		else throw new NoElementFoundException("Element " + toshow + " not found!");
